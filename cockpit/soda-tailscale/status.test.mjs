...
import { connectionState, exitNodeChoices, exitNodeApproval, authenticationURL } from "./status.mjs";
...
test("native connection states remain distinct", () => {
  for (const [status, expected] of [
    ...
    [{ BackendState: "NeedsLogin", AuthURL: "https://login.tailscale.com/a/test" }, "Waiting for browser authentication"],
...
test("eligible exit nodes and advertisement approval use native facts", () => {
  ...
  assert.deepEqual(exitNodeChoices({ Peer: { a: approved, b: { ExitNodeOption: false }, c: { ExitNodeOption: true, Expired: true } } }), [approved]);
  const prefs = { AdvertiseRoutes: ["0.0.0.0/0", "::/0"] };
  assert.equal(exitNodeApproval({ Self: { InNetworkMap: true } }, prefs), "Waiting for Tailnet administrator approval");
  assert.equal(exitNodeApproval({ Self: { InNetworkMap: true, ExitNodeOption: true } }, prefs), "Approved by the Tailnet");
...
test("authentication links never execute page script", () => {
  ...
  assert.equal(authenticationURL("https://login.tailscale.com/a/test"), "https://login.tailscale.com/a/test");
