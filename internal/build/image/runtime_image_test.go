...
func TestRuntimeImageBootcContainerContract(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"COPY --from=rpm-inputs /soda-cosign-*.rpm /var/tmp/soda-rpms/",
		"cosign verify --help",
...
func TestRuntimeImageRPMStagingContract(t *testing.T) {
	...
	require.NotContains(t, string(staging), `b.path("packaging/rpm/runtime/sources/soda-local-access"), filepath.Join(sources, "soda-local-access")`)
	require.NotContains(t, string(staging), `b.path("packaging/rpm/runtime/sources/firewalld/zones/soda-tailnet.xml"), filepath.Join(sources, "soda-tailnet.xml")`)
...
func TestRuntimeHostCompositionRPMContract(t *testing.T) {
	...
	require.NotContains(t, string(runtimeSpec), "install -m 0755 %{_sourcedir}/soda-local-access %{buildroot}%{_bindir}/soda-local-access")
	require.NotContains(t, string(runtimeSpec), "install -m 0755 %{_sourcedir}/soda-setup %{buildroot}%{_libexecdir}/soda/soda-setup")
	require.NotContains(t, string(runtimeSpec), "install -m 0644 %{_sourcedir}/soda-tailnet.xml %{buildroot}%{_sysconfdir}/firewalld/zones/soda-tailnet.xml")
	require.NotContains(t, string(runtimeSpec), "%config(noreplace) %{_sysconfdir}/firewalld/zones/soda-tailnet.xml")
...
func TestForgejoPackagingContract(t *testing.T) {
	...
	require.Contains(t, string(configuration), "DISABLE_REGISTRATION = false")
...
func TestForgejoTailnetPackagingContract(t *testing.T) {
	...
	require.NoError(t, err)
	for _, expected := range []string{
		"/usr/libexec/soda/forgejo-tailnet", "identity=$(hostnamectl --static)", "address=0.0.0.0", "port=30000", "root_url=http://${identity}:${port}/",
		"HTTP_ADDR = ${address}", "HTTP_PORT = ${port}", "DOMAIN = ${identity}", "SSH_DOMAIN = ${identity}", "ROOT_URL = ${root_url}",
	} {
		require.Contains(t, string(initialization), expected)
	}
	require.NotContains(t, string(initialization), "identity=localhost")
...
func TestRuntimeImageSystemdHostCompositionContract(t *testing.T) {
	...
	_, err = os.Stat(filepath.Join(runtimeSources, "systemd", "soda-setup.service"))
	require.ErrorIs(t, err, os.ErrNotExist)
	require.NotContains(t, string(preset), "soda-setup.service")
