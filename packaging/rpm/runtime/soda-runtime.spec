...
Requires:       ca-certificates, gcc, gcc-c++, git-core, iproute, make, nftables-services, openssh-clients, openssh-server, policycoreutils, policycoreutils-python-utils, pkgconf-pkg-config, shadow-utils, soda-forgejo = 15.0.7, sqlite-libs, systemd, tailscale, tar, unzip, util-linux-core, xz

%description
Privileged project daemon, administrator CLI, and SSH session gateway for Soda OS.

%install
mkdir -p %{buildroot}%{_libexecdir}/soda %{buildroot}%{_prefix}/lib/soda/network %{buildroot}%{_bindir} %{buildroot}%{_unitdir} %{buildroot}%{_unitdir}/getty@tty1.service.d %{buildroot}%{_unitdir}/nftables.service.d %{buildroot}%{_presetdir} %{buildroot}%{_tmpfilesdir} %{buildroot}%{_sysctldir} %{buildroot}%{_sysusersdir} %{buildroot}%{_sysconfdir}/profile.d %{buildroot}%{_sysconfdir}/ssh/sshd_config.d %{buildroot}%{_sysconfdir}/soda/authorized_keys
