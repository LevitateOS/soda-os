...
Requires:       /usr/bin/bootc, /usr/bin/skopeo, /usr/bin/cosign

%description
Tailnet enrollment, OpenSSH, firewall, console guidance, and upstream service
composition and administrator-controlled image updates for the Soda OS appliance.
...
mkdir -p %{buildroot}%{_datadir}/cockpit/soda-updates
cp -a %{_sourcedir}/soda-updates-cockpit/. %{buildroot}%{_datadir}/cockpit/soda-updates/
mkdir -p %{buildroot}%{_datadir}/cockpit/soda-tailscale
cp -a %{_sourcedir}/soda-tailscale-cockpit/. %{buildroot}%{_datadir}/cockpit/soda-tailscale/
install -m 0644 %{_sourcedir}/60-soda-tailscale.conf %{buildroot}%{_sysctldir}/60-soda-tailscale.conf

%files
%{_bindir}/soda-tailnet
%{_libexecdir}/soda/soda-updates
%{_datadir}/cockpit/soda-updates/
