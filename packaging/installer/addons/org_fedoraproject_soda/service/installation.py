...
class ProvisionSodaInstallationTask(Task):
    ...
    def _ensure_persistent_bootc_var(self):
        ...
        try:
            ...
            raise RuntimeError(
                "the persistent bootc variable-data root is unavailable"
            ) from None
        ...
        if result.returncode != 0:
            raise RuntimeError(
                "the persistent bootc variable-data root could not be mounted"
            )
        try:
    ...
    def _label_installed_linux_account(self, username):
        ...
        if not home_contexts.is_file():
            raise RuntimeError("the installed SELinux home policy is unavailable")
